...
import "./Homes.scss";
import WatchTime from "../../components/watch-time/WatchTime";
...
export default function AllHomes() {
  ...
  const homesToDisplay = searchResult.length > 0 ? searchResult : homes;
      ...
      <>
        {isLongLoading && (
          <h2 style={{ textAlign: "center", marginTop: "36px" }}>
            The loading time can be long, because the back end is deployed on
            free service!
          </h2>
        )}
        <Spinner />
  ...
  function handleSubmitResult(e) {
    ...
    const filteredLowerPrice = searchValues.minPrice
      ? filteredByNeighborhood.filter(
          (home) => home.price >= searchValues.minPrice
        )
      : filteredByNeighborhood;
    const filteredBymaxPrice = searchValues.maxPrice
      ? filteredLowerPrice.filter(
          (home) => home.price < Number(searchValues.maxPrice)
        )
      : filteredLowerPrice;
